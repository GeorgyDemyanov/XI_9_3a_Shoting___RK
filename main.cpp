...
int main() {
    ...
    double m = (a1 + a2) / 2;
    double end = 1;
    std::vector<std::pair<double, double>> sol = solution(0, 1, 0.001, {0, m}, f);
    while (std::fabs(sol[sol.size()-1].second - 2) > 1e-6) {
        if (sol[sol.size() - 1].second > 2) {
            a2 = m;
            m = (a1 + a2) / 2;
        } else {
            if (end < 2) {
        ...
        sol = solution(0, 1, 0.001, {0, m}, f);
    }
    std::ofstream fout("C:\\Users\\Demia\\CLionProjects\\XI 9.3a Shoting + RK\\data.txt");
